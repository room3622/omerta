...
ko.bindingHandlers.timerButton = {
    ...
    update: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
        // This will be called once when the binding is first applied to an element,
        // and again whenever any observables/computeds that are accessed change
        // Update the DOM element based on the supplied values here.Â«
    }
...
omerta.model.Character = function (oData) {
    ...
    this.name = ko.observable(oData.name || '').extend({ maxLength: 12, minLength: 2, required: {message: 'You must choose an ingame before start'} });
