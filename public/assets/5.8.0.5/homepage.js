...
function showfeature() {
    document.getElementById('hp_features').setAttribute('style', 'display:block;');
    document.getElementById('hp_screenshots').style.display = 'none';
    document.getElementById('hp_families').style.display = 'none';
}
function showscreenshots() {
    document.getElementById('hp_features').style.display = 'none';
    document.getElementById('hp_screenshots').setAttribute('style', 'display:block;');
    document.getElementById('hp_families').style.display = 'none';
}
function showfamilies() {
    document.getElementById('featurediv').style.display = 'none';
    document.getElementById('screenshotdiv').style.display = 'none';
    document.getElementById('familydiv').setAttribute('style', 'display:block;');
    document.getElementById('showfamilies').setAttribute('style', 'display:inline-block;margin-right:7px;line-height:42px;width:168px;background:url(/static_ajax/v5/homepage/images/pic_bg-features.png);text-align:center;border-top-left-radius:10px;border-top-right-radius:10px;color:#fff;');
    document.getElementById('showscreenshots').setAttribute('style', 'display:inline-block;margin-right:7px;line-height:42px;width:168px;background:#272727;text-align:center;border-top-left-radius:10px;border-top-right-radius:10px;filter:alpha(opacity=80);-moz-opacity:0.80;opacity:0.80;color:#9F9D9D;');
    document.getElementById('showfeature').setAttribute('style', 'display:inline-block;margin-right:7px;line-height:42px;width:168px;background:#272727;text-align:center;border-top-left-radius:10px;border-top-right-radius:10px;filter:alpha(opacity=80);-moz-opacity:0.80;opacity:0.80;color:#9F9D9D;');
    $(function()
    {
...
function HomepageViewModel() {
	...
	self.checkLogin = function(element) {
		...
		// Send to server
		// server.get('Omerta\\Web\\Homepage\\Main', 'login', _data, self.checkLoginCB, self.checkLoginERR);
	};
	...
	self.setFormErrors = function(status) {
		...
		if (status === true) {
			$(".login").stop(true).effect("shake", {distance: 10, times: 3});
	...
	self.doForgottenPassword = function(element) {
		var _data = $(element).serialize();
		...
		xhr.done(function(data) {
			window.forgotModal.hide();
			var messages = self.messagifyEntries(data.data.message);
	...
	self.promptInline = function(model, event) {
		...
		$.prompt([{title: title, buttons: {"Close": false}, html: $content.html()}]);
	};
